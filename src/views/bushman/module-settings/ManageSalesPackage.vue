...
export default {
  ...
  setup() {
    ...
    const preferred_species = ref([])
    const speciesOptions = ref([])
    const speciesObjects = ref([])
    const huntingTypesOptions = ref([])
    const salesQuotasOptions = ref([])
    ...
    function addNewSpeciesItemToStorage() {
      ...
      const exists = speciesObjects.value.some(
        (species: { species_id: any }) => species.species_id === form.species.value,
      )
      if (!exists) {
