...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const regulatoryPackagesOptions = ref([]);
    ...
    watch(() => props.editItem, (newItem) => {
        if (newItem && props.editMode) {
            ...
            speciesObjects.value = newItem.species.map(s => ({
                id: s.species.id,
                name: s.species.name,
                quantity: s.quantity
            }));
    ...
    async function getAreas() {
      try {
        ...
        areasOptions.value = response.data.map((item) => ({
          value: item.id,
          text: item.name,
        }));
      } catch (error) {
    ...
    async function getLicencePackages() {
      try {
        ...
        regulatoryPackagesOptions.value = response.data.map((item) => ({
          value: item.id,
          text: item.name,
        }));
      } catch (error) {
    ...
    async function getSpeciesBasedOnAreaAndLicence() {
        ...
        try {
            ...
            speciesOptions.value = response.data.map((item) => ({
                value: item.id,
                text: item.name,
            }));
        } catch (error) {
