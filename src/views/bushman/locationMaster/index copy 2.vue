        ...
        <div class="card ">
          ...
          <div class="card-header d-flex align-items-center  bg-white fw-400">
            ...
            <div class="ms-auto d-flex align-items-center gap-2">
              <div class="ms-auto d-flex align-items-center gap-2">
                <!-- <button @click="scheduleTimetableAction"
                  class="btn btn-secondary btn-smtext-decoration-none text-body text-opacity-50">
                  <i class="fa fa-sync-alt me-1"></i> Refresh
                </button> -->
                <!-- <template v-if="activeTab === 'citylink'">
                  <button class="btn btn-primary " @click="openCityLinkModal()">
...
<template v-else>
          ...
          <div class="card-header p-0 border-bottom  bg-white ">
            <ul class="nav nav-tabs w-100 overflow-auto flex-nowrap mt-2">
              ...
              <li class="nav-item  flex-fill text-center">
                <a @click="activeTab = 'stops'; fetchStopsList(); " class="nav-link"
          ...
          <div class="tab-content p-4">
                  ...
                  <div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
                    <div class="panel br-6 p-0">
                      <div class="custom-table p-3">
                        ...
                        <StandardDataTable :columns="routeColumns" :data="routesList" :loading="isLoading"
                          :filters="routeTableOption" :defaultPageSize="routeTableOption.pageSize"
                          :disablePagination="false" :showDateFilters="false" :actionButtons="routepageActions">
                          <template #city_link_id="{ row }">
                            <span>{{ row.city_link_id || 'N/A' }}</span>
                          </template>
                          <template #route_code="{ row }">
                            <span>{{ row.route_code || 'N/A' }}</span>
                          </template>
                          <template #route_distance="{ row }">
                            <span class="badge bg-info text-dark">{{ row.route_distance || 'N/A' }} km</span>
                          </template>
                          <template #approximate_hrs="{ row }">
                            <span class="badge bg-warning text-dark">{{ row.approximate_hrs || 'N/A' }}
                              hrs</span>
                          </template>
                          ...
                          <template #actions="{ row }">
                            <div class="d-flex gap-1">
                              <button class="btn btn-sm btn-outline-secondary me-1" @click="openRouteModal(row)"
                                title="Edit Route">
                ...
                <div class="modal-dialog">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h5 class="modal-title">{{ currentRoute.id ? 'Edit Route' : 'Route Registration Form' }}</h5>
                    ...
                    <div class="modal-footer sticky-bottom bg-white">
                      ...
                      <button v-if="isMultipleRouteMode" type="button" class="btn btn btn-outline-secondary"
                        @click="addRouteRow">
                    ...
                    <div class="panel br-6 p-0">
                      <div class="custom-table p-3">
                        <v-client-table :data="subRoutesList" :columns="subRouteColumns" :options="subRouteTableOption">
                          <template #city_link_id="{ row }">
                            <span>{{ row.city_link_id }}</span>
                          </template>
                          <template #route_id="{ row }">
                            <span>{{ row.route_id }}</span>
                          </template>
                          <template #sequency="{ row }">
                            <span>{{ row.sequency }}</span>
                          </template>
                          <template #is_active="{ row }">
                            <span :class="row.is_active ? 'text-success' : 'text-danger'">
                              {{ row.is_active ? 'Active' : 'Inactive' }}
                          ...
                          <template #actions="{ row }">
                            <button class="btn btn-sm me-1" @click="openSubRouteModal(row)">
                            ...
                            <button class="btn btn-sm" title="Details" @click="showDetails(row, 'subroute')">
                              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
            ...
            <div class="tab-pane fade" :class="{ 'show active': activeTab === 'stops' }" id="icon-stops" role="tabpanel"
              aria-labelledby="icon-stops-tab">
                              ...
                              <tr v-for="(stop, index) in multipleStops" :key="index">
                                ...
                                <td>
                                  <input v-model="stop.name" @input="formatMultipleStopNameToUppercase(index)"
                                    type="text" class="form-control form-control-sm" placeholder="Enter stop name" />
                                </td>
                                <!-- City -->
                                <td>
                                ...
                                <td v-if="multipleStops.length > 1"  class="text-center">
                                  <button type="button"
                                    class="btn btn-sm btn-outline-danger" @click="removeStopRow(index)">
                                    remove
                                  </button> 
                                     </td>
                ...
                <div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
                  <div class="panel br-6 p-0">
                    <div class="custom-table p-3">
                      ...
                      <StandardDataTable :columns="citylinkColumns" :data="cityLinks" :loading="isLoading"
                        :filters="cityLinkTableOption" :defaultPageSize="cityLinkTableOption.pageSize"
                        :disablePagination="false" :showDateFilters="false" :actionButtons="citylinkpageActions">
                        <template #code="{ row }">
                        ...
                        <template #approx_hours="{ row }">
                          <span>{{ row.approx_hours || 'N/A' }}</span>
                        </template>
                        <template #actions="{ row }">
                          <button class="btn btn-sm me-1" @click="openCityLinkModal(row)" title="Edit City Link">
                            <i class="fa fa-edit text-primary"></i>
                          </button>

                          <button class="btn btn-sm" title="Details" @click="showDetails(row, 'citylink')">
                            <i class="fa fa-eye text-info"></i>
                          </button>
                        </template>
                      </StandardDataTable>
              ...
              <div class="modal modal-xl fade" id="cityLinkModal" tabindex="-1">
                <div class="modal-dialog">
                  <div class="modal-content">
                              ...
                              <tr v-for="(link, index) in multipleCityLinks" :key="index">
                                ...
                                <td>
                                  <multiselect v-model="link.original_city_id"
                                    ...
                                    placeholder="Select " @select="onMultipleOriginalCityChange(index)" />
                                </td>
                                ...
                                <td>
                                  <multiselect v-model="link.destination_city_id"
                                    ...
                                    placeholder="Select" @select="onMultipleDestinationCityChange(index)" />
                                </td>
                    ...
                    <div class="modal-footer">
                      <button class="btn btn-outline-danger" data-bs-dismiss="modal">Cancel</button>
                      <button v-if="isMultipleMode" type="button" class="btn btn btn-outline-secondary"
                        @click="addCityLinkRow">
                        <i class="fa fa-plus me-1"></i> Add Row
                      </button>
