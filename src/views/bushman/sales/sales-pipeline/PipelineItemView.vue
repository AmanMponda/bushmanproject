...
const preselectedInquiry = ref<Inquiry | null>(null)
...
const loadItem = async () => {
  ...
  try {
    ...
    const type = (route.query.type as string) || 'proposal'
    ...
    if (type === 'inquiry') {
      ...
      try {
        ...
        await fetchProposalById(itemId)
        if (currentProposal.value) {
          proposal.value = currentProposal.value
        } else {
          error.value = inquiryError.response?.data?.message || 'Failed to load inquiry details'
        }
      }
    } else {
      // Fetch proposal
    ...
    error.value = e.response?.data?.message || 'Failed to load item details'
  } finally {
    isLoading.value = false
  }
}
...
const goBack = () => {
  router.push({ name: 'sales-pipeline' })
}
...
const handleEdit = () => {
  editingProposal.value = proposal.value
...
const handleSaveProposal = async (data: any) => {
  try {
    ...
    toast?.init({
      message: err.response?.data?.message || 'Failed to save quotation',
      color: 'danger',
    })
  }
}

// Watch for route changes and load item when ID is available
watch(
  () => route.params.id,
  (newId, oldId) => {
