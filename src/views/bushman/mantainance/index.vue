        ...
        <div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
          <div class="panel br-6 p-0">
            <div class="custom-table p-3">
              <StandardDataTable
                :columns="columns" :data="dataFetched" :loading="isLoading"
                :filters="tableFilters" :defaultPageSize="tableFilters.pageSize"
                :disablePagination="false" :showDateFilters="false"
              ...
              >
                ...
                <template #actions="{ row }">
                  <div class="d-flex gap-1">
                    <button class="btn btn-info btn-sm" @click="viewMethod(row)" title="View">
                      <i class="fa fa-eye"></i>
                    </button>
                    <button class="btn btn-secondary btn-sm" @click="editMethod(row)" title="Edit">
                      <i class="fa fa-edit"></i>
                    </button>
                    <button class="btn btn-danger btn-sm" @click="deleteMethod(row)" title="Delete">
          ...
          <div class="modal-content border-0 shadow">
            <div class="modal-body text-center p-4">
              ...
              <p class="text-muted small mb-3">
                Delete <strong>{{ categoryToDelete?.name }}</strong>? Cannot be undone.
              </p>
              <div class="d-flex gap-2 justify-content-center">
                <button class="btn btn-outline-secondary btn-sm px-4" data-bs-dismiss="modal">Cancel</button>
                <button class="btn btn-danger btn-sm px-4" @click="confirmDeleteExec" :disabled="deleting">
    ...
    <template v-else-if="currentView === 'wizard'">
      ...
      <div class="d-flex justify-content-between align-items-center mb-2">
        <ul class="breadcrumb mb-0">
          ...
          <li class="breadcrumb-item">
            <a href="#" @click.prevent="backToTable">MAINTENANCE CATEGORIES</a>
          </li>
          <li class="breadcrumb-item active">{{ isEditing ? 'EDIT' : 'CREATE' }}</li>
        </ul>
        <button class="btn btn-outline-secondary btn-sm px-3" @click="backToTable">
      ...
      <div v-if="draft.id && currentStep > 0"
        ...
        <button class="btn btn-outline-secondary btn-xs ms-auto" @click="backToTable">
        ...
        <div class="card border-0 shadow-sm">
          ...
          <div class="card-footer bg-white border-top d-flex justify-content-between align-items-center py-2">
            ...
            <div class="d-flex gap-2">
              <!-- If already saved, let user skip straight to next -->
      ...
      <div v-show="currentStep === 1">
        <div class="card border-0 shadow-sm">
          ...
          <div class="card-footer bg-white border-top d-flex justify-content-between py-2">
            ...
            <div class="d-flex gap-2">
              <button class="btn btn-outline-secondary btn-sm px-4" @click="backToTable">
      ...
      <div v-show="currentStep === 5">
        <div class="card border-0 shadow-sm">
          ...
          <div class="card-footer bg-white border-top d-flex justify-content-between align-items-center py-2">
            ...
            <button class="btn btn-success btn-sm px-5" @click="backToTable">
...
// ── Auth headers — backend reads X-User-Id for created_by / updated_by ──
...
const savedSteps  = ref(new Set());   // steps confirmed saved to DB
...
const freshDraft = () => ({
  ...
  symptoms:   [],
  parameters: [],   // each: { id, name, question_type, unit, active, options:[], results:[] }
});
...
const columns = ref([
  { key: 'sno',         label: '#',           visible: true, sortable: true  },
  { key: 'name',        label: 'Category',    visible: true, sortable: true  },
  { key: 'asset_group', label: 'Asset Group', visible: true, sortable: false },
  { key: 'site',        label: 'Site',        visible: true, sortable: false },
  { key: 'parameters',  label: 'Parameters',  visible: true, sortable: false },
  { key: 'symptoms',    label: 'Symptoms',    visible: true, sortable: false },
...
const pageActions = computed(() => [
  {
    label: 'Refresh', icon: 'fa fa-sync', class: 'btn btn-secondary',
    method: () => fetchData(),
  },
  {
    label: 'New Category', icon: 'fa fa-plus', class: 'btn btn-primary',
    method: () => openWizard(null),
  },
]);
...
// ── Reactive input rows ────────────────────────────────────────
const newOpts     = ref({});   // keyed by param.id
const newRes      = ref({});   // keyed by param.id
const newItemsMap = ref({});   // keyed by result.id
...
const editOptKey   = ref(null);   // "paramId_optId"
const editOptData  = ref({});
const savingOpt    = ref(false);
const addingOptParamId = ref(null);

const editResKey   = ref(null);   // "paramId_resId"
...
// OPEN WIZARD
// ══════════════════════════════════════════════════════════════
const openWizard = async (row = null) => {
  errors.value    = {};
  savedSteps.value = new Set();
  currentStep.value = 0;
  newOpts.value = {}; newRes.value = {}; newItemsMap.value = {};
  await Promise.all([fetchAssetsGroup(), fetchSites(), fetchItems()]);

  if (row) {
    isEditing.value = true;
    try {
      const { data } = await axiosInstance.get(`/maintenance/categories/${row._raw.id}`, {
    ...
    } catch { showAlert('error', 'Failed to load category.'); return; }
  } else {
...
// ══════════════════════════════════════════════════════════════
// STEP 0 — Save / update category to backend immediately
// ══════════════════════════════════════════════════════════════
...
// ── Table methods ──────────────────────────────────────────────
const viewMethod   = (row) => router.push(`/module-settings/inspection-settings/view/${row._raw.id}`);
const editMethod   = (row) => openWizard(row);
const deleteMethod = (row) => {
  categoryToDelete.value = row._raw;
  bootstrap.Modal.getOrCreateInstance(deleteModalRef.value).show();
};
const confirmDeleteExec = async () => {
  deleting.value = true;
  try {
    await axiosInstance.delete(`/maintenance/categories/${categoryToDelete.value.id}`);
    showAlert('success', 'Deleted successfully.');
    bootstrap.Modal.getOrCreateInstance(deleteModalRef.value).hide();
    await fetchData();
...
const fetchData = async () => {
  ...
  try {
    const res = await axiosInstance.get('/maintenance/categories');
    const list = res.data.data ?? res.data;
    dataFetched.value = list.map((d, i) => ({
      sno:         i + 1,
      name:        d.name,
      asset_group: d.asset_group?.name ?? '—',
      site:        d.site?.name        ?? 'Global',
      parameters:  String(d.parameters_count ?? d.parameters?.length ?? 0),
      symptoms:    String(d.symptoms_count   ?? d.symptoms?.length   ?? 0),
      status:      d.deleted_at ? 'Inactive' : 'Active',
      _raw: d,
    }));
